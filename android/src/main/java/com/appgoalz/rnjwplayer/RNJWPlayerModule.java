...
public class RNJWPlayerModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  public void changeMuteStatus(final int reactTag, final boolean isMuted) {
    System.out.println("Got here in mute status change");
    try {
      UIManagerModule uiManager = mReactContext.getNativeModule(UIManagerModule.class);
      uiManager.addUIBlock(new UIBlock() {
        public void execute (NativeViewHierarchyManager nvhm) {
          ...
          if (playerView != null && playerView.mPlayerView != null) {
            ...
            if(isMuted){
              playerView.mPlayerView.getPlayer().setMute(false);
              playerView.mPlayerView.getPlayer().setVolume(100);
            }else{
              playerView.mPlayerView.getPlayer().setVolume(0);
            }

            // if(playerView.mPlayerView.getPlayer().getMute() == false){
            //     playerView.mPlayerView.getPlayer().setVolume(0);
            // }else{
            //   playerView.mPlayerView.getPlayer().setMute(false);
            //   playerView.mPlayerView.getPlayer().setVolume(100);
